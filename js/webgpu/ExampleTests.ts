...
const testBicDoubleScan = ( options: MultilevelTestScanOptions ) => {
  ...
  asyncTestWithDevice( name, async ( device, deviceContext ) => {
    ...
    const shader = await DoubleReduceScanShader.create<Bic>( deviceContext, name, {
      workgroupSize: workgroupSize,
      grainSize: grainSize,
      isCommutative: false,
      valueType: 'vec2u',
...
const testBicTripleScan = ( options: MultilevelTestScanOptions ) => {
  ...
  asyncTestWithDevice( name, async ( device, deviceContext ) => {
    ...
    const shader = await TripleReduceScanShader.create<Bic>( deviceContext, name, {
      workgroupSize: workgroupSize,
      grainSize: grainSize,
      isCommutative: false,
      valueType: 'vec2u',
...
asyncTestWithDevice( 'u32 triple radix sort', async ( device, deviceContext ) => {
  ...
  // const inputValues = [ 4, 9, 11, 15, 15, 16, 22, 24, 24, 29, 35, 38, 38, 41, 49, 53, 68, 79, 88, 89, 89, 90, 92, 94, 94, 112, 112, 122, 124, 129, 137, 159, 161, 164, 177, 177, 184, 186, 187, 187, 194, 201, 211, 212, 214, 219, 223, 227, 229, 233, 238, 240, 247 ];
  // const inputValues = [
