...
asyncTestWithDevice( 'u32 triple radix sort - broken?', async ( device, deviceContext ) => {
  const totalBits = 32;
  const bitQuantity = 3;
  const innerBitQuantity = 2;
  ...
  const shader = await TripleRadixSortShader.create<number>( deviceContext, 'u32 triple radix sort - broken?', {
    valueType: 'u32',
    totalBits: totalBits,
    getBits: ( value: string, bitOffset: number, bitQuantity: number ) => {
      return `( ( ${value} >> ${u32( bitOffset )} ) & ${u32( ( 1 << bitQuantity ) - 1 )} )`;
