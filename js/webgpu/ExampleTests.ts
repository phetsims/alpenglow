...
import { Binding, ComputeShader, DeviceContext, wgsl_reduce_raked_blocked_single, wgsl_reduce_raked_striped_single, wgsl_reduce_simple_single, wgsl_scan_simple_single } from '../imports.js';
...
asyncTestWithDevice( 'scan_simple_single', async device => {
  const workgroupSize = 256;
  ...
  const shader = ComputeShader.fromSource(
    device, 'scan_simple_single', wgsl_scan_simple_single, [
      Binding.READ_ONLY_STORAGE_BUFFER,
      Binding.STORAGE_BUFFER
    ], {
      workgroupSize: workgroupSize
...
asyncTestWithDevice( 'reduce_raked_blocked_single', async device => {
  const workgroupSize = 256;
  const grainSize = 4;
  ...
  const shader = ComputeShader.fromSource(
    device, 'reduce_raked_blocked_single', wgsl_reduce_raked_blocked_single, [
      Binding.READ_ONLY_STORAGE_BUFFER,
      Binding.STORAGE_BUFFER
    ], {
      workgroupSize: workgroupSize,
...
asyncTestWithDevice( 'reduce_raked_striped_single', async device => {
  const workgroupSize = 256;
  const grainSize = 4;
  ...
  const shader = ComputeShader.fromSource(
    device, 'reduce_raked_striped_single', wgsl_reduce_raked_striped_single, [
      Binding.READ_ONLY_STORAGE_BUFFER,
      Binding.STORAGE_BUFFER
    ], {
      workgroupSize: workgroupSize,
