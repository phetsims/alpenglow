...
asyncTestWithDevice( 'f32_reduce_simple', async ( device, deviceContext ) => {
  ...
  const shader = await OldExampleSimpleF32Reduce.create( deviceContext, 'f32_reduce_simple', {
...
const fullAtomicReduceTest = ( valueType: AtomicType, atomicOperation: AtomicOperation, directAtomics: boolean, jsOperation: ( a: number, b: number ) => number, jsIdentity: number ) => {
  const name = `${valueType} FULL atomic ${atomicOperation} reduce direct:${directAtomics}`;
  asyncTestWithDevice( name, async ( device, deviceContext ) => {
    const workgroupSize = 256;
    const grainSize = 8;
    const numAtomics = 8;
    ...
    const shader = await FullAtomicReduceShader.create( deviceContext, name, {
      valueType: valueType,
      atomicOperation: atomicOperation,
      workgroupSize: workgroupSize,
      grainSize: grainSize,
...
[ false, true ].forEach( directAtomics => {
  fullAtomicReduceTest( 'u32', 'atomicAdd', directAtomics, ( a, b ) => a + b, 0 );
  fullAtomicReduceTest( 'u32', 'atomicMax', directAtomics, ( a, b ) => Math.max( a, b ), 0 );
  fullAtomicReduceTest( 'u32', 'atomicMin', directAtomics, ( a, b ) => Math.min( a, b ), Number.POSITIVE_INFINITY );
  fullAtomicReduceTest( 'u32', 'atomicAnd', directAtomics, ( a, b ) => a & b, 0xffffffff );
  fullAtomicReduceTest( 'u32', 'atomicOr', directAtomics, ( a, b ) => a | b, 0 );
  fullAtomicReduceTest( 'u32', 'atomicXor', directAtomics, ( a, b ) => a ^ b, 0 );

  fullAtomicReduceTest( 'i32', 'atomicAdd', directAtomics, ( a, b ) => a + b, 0 );
  fullAtomicReduceTest( 'i32', 'atomicMax', directAtomics, ( a, b ) => Math.max( a, b ), Number.NEGATIVE_INFINITY );
  fullAtomicReduceTest( 'i32', 'atomicMin', directAtomics, ( a, b ) => Math.min( a, b ), Number.POSITIVE_INFINITY );
...
const testBicRakedReduce = ( combineWithExpression: boolean, convergent: boolean, inputOrder: 'blocked' | 'striped', inputAccessOrder: 'blocked' | 'striped' ) => {
  ...
  asyncTestWithDevice( name, async ( device, deviceContext ) => {
    ...
    const bics = _.range( 0, fullInputSize ).map( () => new OldBic( random.nextIntBetween( 1, 64 ), random.nextIntBetween( 1, 64 ) ) );
    ...
    const actualNumbers = await deviceContext.executeSingle( async ( encoder, execution ) => {
      ...
      const outputBuffer = execution.createBuffer( OldBic.BYTES * dispatchSize );
    ...
    const actualBics: OldBic[] = [];
    for ( let i = 0; i < actualNumbers.length - 1; i += 2 ) {
      actualBics.push( new OldBic( actualNumbers[ i ], actualNumbers[ i + 1 ] ) );
    }

    const expectedBics = _.chunk( bics.slice( 0, inputSize ), blockSize ).map( bicList => OldBic.combineMultiple( ...bicList ) );
...
const testSingleScan = <T>( binaryOp: BinaryOp<T>, exclusive: boolean ) => {
  const name = `main_scan (SingleScanShader) ${binaryOp.name} single exclusive:${exclusive}`;
  asyncTestWithDevice( name, async ( device, deviceContext ) => {
    const workgroupSize = 256;
    const grainSize = 8;
    const inputSize = workgroupSize * grainSize * 5 - 27;

    const shader = await SingleScanShader.create<T>( deviceContext, name, {
      binaryOp: binaryOp,
      workgroupSize: workgroupSize,
      grainSize: grainSize,
      lengthExpression: u32( inputSize ),
    ...
    const inputValues = _.range( 0, inputSize ).map( () => binaryOp.type.generateRandom() );

    const actualValues = await deviceContext.executeShader( shader, inputValues );
    const expectedValues = _.chunk( inputValues.slice( 0, inputSize ), workgroupSize * grainSize ).flatMap( values => {
      const result: T[] = [];
      let value = binaryOp.identity;
      for ( let i = 0; i < values.length; i++ ) {
        exclusive && result.push( value );

        value = binaryOp.apply( value, values[ i ] );
...
testSingleScan( U32Add, false );
testSingleScan( U32Add, true );
testSingleScan( Vec2uBic, false );
testSingleScan( Vec2uBic, true );
...
const testU32DoubleScan = ( options: MultilevelTestScanOptions ) => {
  ...
  asyncTestWithDevice( name, async ( device, deviceContext ) => {
    ...
    const shader = await DoubleReduceScanShader.create<number>( deviceContext, name, {
      workgroupSize: workgroupSize,
      grainSize: grainSize,
      valueType: 'u32',
      isCommutative: true,
...
const testBicDoubleScan = ( options: MultilevelTestScanOptions ) => {
  ...
  asyncTestWithDevice( name, async ( device, deviceContext ) => {
    ...
    const shader = await DoubleReduceScanShader.create<OldBic>( deviceContext, name, {
      workgroupSize: workgroupSize,
      grainSize: grainSize,
      isCommutative: false,
      valueType: 'vec2u',
      bytesPerElement: OldBic.BYTES,
      identityExpression: 'vec2( 0u )',
      combineExpression: ( a: string, b: string ) => `( ${a} + ${b} - min( ${a}.y, ${b}.x ) )`,
      encodeElement: ( bic: OldBic, encoder: ByteEncoder ) => {
        encoder.pushU32( bic.x );
        encoder.pushU32( bic.y );
      },
      decodeElement: ( encoder: ByteEncoder, offset: number ) => new OldBic( encoder.fullU32Array[ offset ], encoder.fullU32Array[ offset + 1 ] ),
      lengthExpression: u32( inputSize ),
      inputOrder: 'blocked',
      inputAccessOrder: options.inputAccessOrder,
      exclusive: options.exclusive,
    ...
    const bics = _.range( 0, inputSize ).map( () => new OldBic( random.nextIntBetween( 0, 63 ), random.nextIntBetween( 0, 63 ) ) );

    const actualValue = await deviceContext.executeShader( shader, bics );

    const expectedValue: OldBic[] = [];
    let value = OldBic.IDENTITY;
    for ( let i = 0; i < inputSize; i++ ) {
      options.exclusive && expectedValue.push( value );

      value = OldBic.combine( value, bics[ i ] );
...
const testU32TripleScan = ( options: MultilevelTestScanOptions ) => {
  ...
  asyncTestWithDevice( name, async ( device, deviceContext ) => {
    ...
    const shader = await TripleReduceScanShader.create<number>( deviceContext, name, {
      workgroupSize: workgroupSize,
      grainSize: grainSize,
      valueType: 'u32',
      isCommutative: true,
...
const testBicTripleScan = ( options: MultilevelTestScanOptions ) => {
  ...
  asyncTestWithDevice( name, async ( device, deviceContext ) => {
    ...
    const shader = await TripleReduceScanShader.create<OldBic>( deviceContext, name, {
      workgroupSize: workgroupSize,
      grainSize: grainSize,
      isCommutative: false,
      valueType: 'vec2u',
...
// Just do CompareOrder for easier testing
